...
void MPU9250::setWire(TwoWire *thewire) {
  myWire = thewire;
}
...
uint8_t MPU9250::writeByteWire(uint8_t deviceAddress, uint8_t registerAddress,
                            uint8_t data)
{
  (myWire)->beginTransmission(deviceAddress);  // Initialize the Tx buffer
  (myWire)->write(registerAddress);      // Put slave register address in Tx buffer
  (myWire)->write(data);                 // Put data in Tx buffer
  (myWire)->endTransmission();           // Send the Tx buffer
                        ...
                        uint8_t count, uint8_t * dest)
{
  ...
  (myWire)->requestFrom(deviceAddress, count);
